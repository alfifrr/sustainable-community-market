...
def get_item_history():
    ...
    product_history = ItemTransaction.query.filter(db.or_(
        ItemTransaction.buyer_id == current_user_id,
        ItemTransaction.seller_id == current_user_id
    )).order_by(ItemTransaction.updated_at.desc()).all()
...
def get_products():
    transaction_history = TransactionHistory.query.filter_by(
        user_id=get_jwt_identity()).order_by(TransactionHistory.date.desc()).all()
